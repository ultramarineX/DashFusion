...
class PositionEncodingTraining(nn.Module):
    ...
    def __init__(self, fea_size=None, tf_hidden_dim=None, drop_out=None, dataset=None, config=default_config):
        ...
        if drop_out is None:
            drop_out = config.PARAM.downStream.a_drop_out
        
        if dataset == 'mosi':
            num_patches = 375
            if fea_size is None:
                fea_size = config.MOSI.audio_fea_dim
        elif dataset == 'mosei':
            num_patches = 500
            if fea_size is None:
                fea_size = config.MOSEI.audio_fea_dim
        elif dataset  == 'sims':
            num_patches = 400
            if fea_size is None:
                fea_size = config.SIMS.audio_fea_dim
...
class AudioEncoder(nn.Module):
    def __init__(self, name=None, dataset=None, fea_size=None, encoder_fea_dim=None, nhead=None, dim_feedforward=None,
                 num_layers=None, drop_out=0.5, config=default_config):
        ...
        if nhead is None:
            nhead = config.PARAM.downStream.audio_nhead
        if drop_out is None:
            drop_out = config.PARAM.downStream.a_drop_out
        if dim_feedforward is None:
            dim_feedforward = config.PARAM.downStream.encoder_fea_dim * 2
        if num_layers is None:
            num_layers = config.PARAM.downStream.audio_tf_num_layers
        if fea_size is None:
            if dataset == 'mosi':
                fea_size = config.MOSI.audio_fea_dim   
            elif dataset == 'mosei':
